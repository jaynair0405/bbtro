 ...
 *
 * One shape, four consumers. buildDataset() is what the API serves; the pages
 * derive the train index and block totals from it client-side using the SAME
 * module the server would use (public/div/suburban/js/sub-derive.js — see the
 * header there for why it lives under public/).
 *
 * buildSnapshots() exists only to keep scripts/build_page_snapshots.js working
 * during the mockup->live migration. It projects the canonical shapes back into
 * the eight legacy literals, KEY ORDER INCLUDED, so the regenerated mockups are
 * byte-identical and can serve as the parity gate. It goes away with the mockups.
 */
'use strict';
...
/**
 * The eight legacy literals, projected from the canonical shapes.
 * MIGRATION ONLY — deleted with the mockups. Key order is deliberate: the
 * regenerated pages must be byte-identical to the previous build.
 */
async function buildSnapshots(pool) {
  const ds = await buildDataset(pool);

  // DETAILS/CYCLES: canonical minus the appended blk
  const DETAILS = ds.details.map(({ blk, ...rest }) => rest);           // eslint-disable-line no-unused-vars
  // LEGS_FULL: canonical minus the appended rmk
  const LEGS_FULL = ds.legs.map(({ rmk, ...rest }) => rest);            // eslint-disable-line no-unused-vars
  const LEGS_BOOK = ds.legs.map((l) => ({
    es: l.es, et: l.et, ss: l.ss, st: l.st, tn: l.tn, ty: l.ty, did: l.did,
    rem: D.bookRemarks(l),
  }));
  // BLOCKS: the dashboard literal drops the appended id/sn/en and hides empties
  const BLOCKS = ds.blocks.filter((b) => b.n > 0)
    .map(({ id, sn, en, ...rest }) => rest);                            // eslint-disable-line no-unused-vars

  const { TRAINS, TDET, TLEGS } = D.buildTrainIndex(ds.details, ds.legs, ds.master);

  return { BLOCKS, CYCLES: DETAILS, LEGS_BOOK, DETAILS, LEGS_FULL, TRAINS, TDET, TLEGS,
    counts: ds.counts, warnings: ds.warnings };
}

module.exports = {
  fetchRaw, buildDetails, buildLegs, buildMasterSlice, checkWarnings,
  buildDataset, buildSummary, buildSnapshots,
};
