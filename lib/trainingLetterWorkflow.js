...
async function course(c,ruleId,centreId,active=true) {
    ...
    [r.renewals]=await c.query(`SELECT rr.*,t.target_name,t.legacy_training_id FROM div_training_rule_renewals rr JOIN div_training_renewal_targets t ON t.target_id=rr.target_id WHERE rr.rule_id=? ORDER BY t.target_id`,[r.rule_id]);
    return r;
}
