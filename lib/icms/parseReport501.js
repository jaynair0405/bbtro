...
const COLUMNS = {
    ...
    atd:       { names: [['actsource', 'atd']],
                 shape: /^\d{1,2}\s+\w{3}\s+\d{2}:\d{2}$/, required: false },
    // Arrival at the destination. On a TERMINATING report this — not STD — is
    // the date the working belongs to: the train left a distant origin one or
    // two days earlier and reaches our terminal on the report date.
    sta:       { names: [['schdestination', 'sta']],
                 shape: /^\d{1,2}\s+\w{3}\s+\d{2}:\d{2}$/, required: false },
    ata:       { names: [['actdestination', 'ata']],
                 shape: /^\d{1,2}\s+\w{3}\s+\d{2}:\d{2}$/, required: false },
...
function parseReport501(input) {
    ...
    // Pick the sheet deliberately rather than trusting index 0 — an export can
    // carry more than one. Prefer one whose header actually contains a train
    // number column; fall back to the first only if nothing else qualifies.
    const sheetName = wb.SheetNames.find((n) => {
        const cand = wb.Sheets[n];
        if (!cand || !cand['!ref']) return false;
        const rng = xlsx.utils.decode_range(cand['!ref']);
        for (let r = rng.s.r; r <= Math.min(rng.s.r + 14, rng.e.r); r++) {
            for (let c = rng.s.c; c <= rng.e.c; c++) {
                const cell = cand[xlsx.utils.encode_cell({ r, c })];
                if (cell && key(cell.w !== undefined ? cell.w : cell.v) === 'trainno') return true;
            }
        }
        return false;
    }) || wb.SheetNames[0];
    ...
    // ORIGINATING vs TERMINATING changes two things, and getting either wrong
    // puts real locos on the wrong rows:
    //
    //   Originating (DN) — the train STARTS at our terminal. Its date is the
    //     scheduled departure, and the departure loco is the one it leaves with.
    //   Terminating (UP) — the train ENDS at our terminal, having left a
    //     distant origin one or two days earlier. Its date is the ARRIVAL, and
    //     the loco that matters is the one it ARRIVES with: 12 of 77 rows on
    //     the 20-Sep file changed loco en route.
    //
    // The title says which. It is confirmed below against the rows themselves,
    // because a mislabelled file would be worse than an unlabelled one.
    const scopeMatch = /\[(Originating|Terminating)\]/i.exec(titleCell);
    let scope = scopeMatch ? scopeMatch[1].toUpperCase() : null;
    ...
    // CONFIRM THE SCOPE FROM THE ROWS, not just the title.
    // An originating report has OUR stations in the source column; a
    // terminating one has them in the destination. Count both and let the data
    // decide — a mislabelled or untitled file would otherwise be dated and
    // loco'd the wrong way round, silently.
    const OUR_STATIONS = new Set(['CSMT', 'LTT', 'DR', 'PNVL', 'DIVA', 'KYN',
                                  'TNA', 'PEN', 'ROHA', 'IGP', 'DI', 'VVH', 'BIRD', 'DW']);
    {
        const srcIdx = columns.src ? columns.src.index : -1;
        const dstIdx = columns.dstn ? columns.dstn.index : -1;
        let srcHits = 0, dstHits = 0;
        if (srcIdx >= 0 || dstIdx >= 0) {
            for (let r = header.fieldRow + 1; r < grid.length; r++) {
                const raw = grid[r];
                if (!COLUMNS.train_no.shape.test((raw[ti] || '').trim())) continue;
                if (srcIdx >= 0 && OUR_STATIONS.has((raw[srcIdx] || '').trim().toUpperCase())) srcHits++;
                if (dstIdx >= 0 && OUR_STATIONS.has((raw[dstIdx] || '').trim().toUpperCase())) dstHits++;
            }
        }
        const fromData = (srcHits === dstHits) ? null
                       : (dstHits > srcHits ? 'TERMINATING' : 'ORIGINATING');
        if (fromData && scope && fromData !== scope) {
            // Trust the rows. The title is a label; the stations are the fact.
            warnings.push(`The title says [${scope}] but ${dstHits > srcHits ? 'destinations' : 'sources'} are our own stations (${srcHits} source / ${dstHits} destination matches), which is a ${fromData} report. Reading it as ${fromData}.`);
            scope = fromData;
        } else if (!scope && fromData) {
            scope = fromData;
            warnings.push(`The title does not say Originating or Terminating; the stations make it ${fromData}.`);
        } else if (!scope && !fromData) {
            throw new Error('Cannot tell whether this is an Originating or a Terminating report — the title does not say and the stations are inconclusive. Reading it either way would put the wrong locos on the wrong dates.');
        }
    }
    const terminating = scope === 'TERMINATING';
    ...
    for (let r = header.fieldRow + 1; r < grid.length; r++) {
        ...
        // Terminating: read the ARRIVAL list — that is the engine that reaches
        // our terminal and that the crew takes over.
        const depList = parseLocoList(get('loco_dep'));
        const arrList = parseLocoList(get('loco_arr'));
        const locos = terminating ? (arrList.length ? arrList : depList) : depList;
        // Flagged, not interpreted: ICMS does not say whether a second loco is
        // assisting or dead in tow, so the LPC is shown the change and decides.
        const changedEnRoute = Boolean(
            depList.length && arrList.length &&
            depList.map((l) => l.number).join(',') !== arrList.map((l) => l.number).join(','));

        // Terminating: the working belongs to the arrival date, not departure.
        const workingDate = terminating
            ? rowDate(get('sta'), get('ata'), reportDate, reportDateTo)
            : rowDate(get('std'), get('atd'), reportDate, reportDateTo);
        ...
        rows.push({
            ...
            locos_departed: depList,
            loco_changed_enroute: changedEnRoute,
    ...
    const counts = {
        ...
        loco_changed_enroute: rows.filter(r => r.loco_changed_enroute).length,
    };

    return { reportDate, reportDateTo, dates, mode, scope,
             direction: terminating ? 'UP' : 'DN',
             sheetName, rows, warnings, columns, counts };
}
