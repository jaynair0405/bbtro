...
(() => {
  ...
  const state={config:null,letterId:null,revision:null,nominees:[],locked:false,historical:false,results:[],due:[],dueTotal:0,dueTruncated:false};
  ...
  function dueBadge(row){
    if(row.due_state==='never')return '<span class="warning">Not recorded</span>';
    if(row.due_state==='overdue')return `<span class="warning">Overdue by ${row.days_overdue} day${row.days_overdue===1?'':'s'}</span>`;
    return `<span class="meta">Due ${esc(row.due_date)}</span>`;
  }
  function renderDue(){
    const data=state.due;
    if(!data.length){$('due').innerHTML='<p class="empty">Nobody falls due in this window.</p>';$('due-count').textContent='0';return}
    $('due-count').textContent=state.dueTruncated?`${data.length} of ${state.dueTotal}`:String(data.length);
    $('due').innerHTML=data.map((p,i)=>{
      const on=state.nominees.some(n=>(n.source+':'+n.source_id)===(p.source+':'+p.source_id));
      return `<div class="row"><div><strong>${esc(p.name)}</strong><div class="meta">${esc(p.cms_id||'\u2014')} \u00b7 ${esc(p.lobby||'\u2014')} \u00b7 last ${esc(p.last_training_date||'not recorded')}</div>${dueBadge(p)}</div><button class="btn outline" data-due="${i}"${on?' disabled':''}>${on?'Added':'Add'}</button></div>`;
    }).join('');
  }
  async function loadDue(){
    const button=$('due-load');button.disabled=true;
    try{
      const res=await api('/due?'+new URLSearchParams({rule_id:$('course').value,within:$('due-window').value}));
      state.due=res.data;state.dueTotal=res.total;state.dueTruncated=!!res.truncated;
      $('due-note').textContent=res.note||(res.target?`Falling due for ${res.target}, most overdue first.`+(res.truncated?` Showing the first ${res.data.length}.`:''):'');
      renderDue();
    }catch(e){toast(e.message,true)}finally{button.disabled=false}
  }
  function addFromDue(index){
    const p=state.due[index];if(!p)return;
    const key=p.source+':'+p.source_id;
    if(state.nominees.some(n=>(n.source+':'+n.source_id)===key))return toast('Already selected',true);
    state.nominees.push(p);render();renderDue();
  }
  ...
  function add(index){const p=state.results[index];if(!p)return false;const key=p.source+':'+p.source_id;if(state.nominees.some(n=>(n.source+':'+n.source_id)===key)){toast('Already selected',true);return false}state.nominees.push(p);render();if(state.due.length)renderDue();return true}
  ...
  $('new-letter').addEventListener('click',newLetter);$('history-button').addEventListener('click',history);$('due-load').addEventListener('click',loadDue);$('due-window').addEventListener('change',()=>{if(state.due.length)loadDue()});$('due').addEventListener('click',e=>{const b=e.target.closest('[data-due]');if(b)addFromDue(Number(b.dataset.due))});$('search').addEventListener('input',searchSoon);$('source').addEventListener('change',search);$('search').addEventListener('keydown',e=>{
  ...
  });$('save-letter').addEventListener('click',save);['course','number','letter-date','training-date','requirements'].forEach(id=>$(id).addEventListener('input',()=>{if(id==='course'){if(!state.letterId)$('requirements').value=course()?.requirements_text||'';refreshNominees();if(state.results.length)search();if(state.due.length)loadDue()}render()}));
  $('results').addEventListener('click',e=>{const b=e.target.closest('[data-add]');if(b)addAndContinue(Number(b.dataset.add))});$('nominees').addEventListener('click',e=>{const b=e.target.closest('[data-remove]');if(b){state.nominees.splice(Number(b.dataset.remove),1);render();if(state.due.length)renderDue()}});$('history').addEventListener('click',e=>{const b=e.target.closest('button');if(b?.dataset.letter)loadLetter(b.dataset.letter);if(b?.dataset.version)loadLetter(state.letterId,b.dataset.version)});init();
})();
