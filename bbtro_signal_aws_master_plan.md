...
Each running line is a DN + UP section pair unless noted. Ghats also carry a bidirectional MIDDLE line (modelled as `DN MID` / `UP MID` `line` values).
...
| SE | KJT-LNL (ghat, incl. MID line) | ⬜ pending curation |
| SE | LNL-PUNE | ⬜ pending curation |
| NE | KYN-KSRA DN/UP NE | ✅ imported |
| NE | KSRA-IGP (ghat, incl. UP/DN MID line) | ⬜ pending curation |
| NE | IGP-MMR (→ JL) | ⬜ pending curation |
| — | PNVL_GOODS beat sections | ⬜ none bound yet |
| — | RHS-summary / sidings / station-sequence pages | ⬜ different layout, deferred |

AWS-critical plain sections (KYN-KSRA, KYN-KJT) are done. Ghats + beyond come from the KYN_GOODS / CSMT_ML_JL books. `VSD S-4` (DN NE loop-line starter) deliberately omitted — to be added later as a parallel signal.
...
| Phase 2 (planning) | Ghat sections model the Middle line as two `line` values (`DN MID` / `UP MID`), NOT `direction=BOTH` | Mid-line signals are physically direction-specific (a signal faces one way), so they fit `(section, line, direction)` exactly like `DN ML`/`UP ML`. No schema change. A ghat section (e.g. KSRA-IGP) becomes 4 partitions: DN ML / UP ML / DN MID / UP MID. The book's "middle line as a column" is a rendering choice, deferred — render MID as its own sub-section for now. AWS work needs only the plain KYN-KSRA / KYN-KJT sections, so the ghat is not on the AWS critical path. |
