...
| NE | MMR-BSL (Manmad→Bhusaval, NE trunk continues: DN NE + UP NE, no MID) | ✅ imported → CSMT_ML_MMR only |
| Branch | MMR-SNSI (Manmad→Sainagar Shirdi branch off the NE trunk, DN SNSI + UP SNSI) | ✅ imported → CSMT_ML_MMR only |
...
| Branch | PNVL-JNPT (Panvel→JNPT port freight, DN JNPT + UP JNPT, no MID) | ✅ imported → PNVL_GOODS |
...
| BPT | CLA-VDLR (Kurla→Wadala Bombay Port Trust line, BPT UP + BPT DN) | ✅ imported → KYN_GOODS |
...
| Phase 2 (2026-07-31) | FULL magnet_id backfill (all imported signals) — DONE on LOCAL | `sql/2026-07-31_magnet_id_backfill_full.sql`. Backfilled all 1087 this-session NULL magnet_id rows (→ 0 NULL). Identity rule `(station, signal_number, direction)` = one magnet. Steps: (1) station-coded → group by (station_code, number, direction), NULL copies adopt existing twin's magnet else group MIN(id) — covers singletons, within-section MID/line dups, and 85 cross-section boundary groups (Lonavala/Pen/Roha/Igatpuri/DCC S-27-28/DI S-5/BPT CLA-yard etc.); (2) blank-station automatics → group by (number, direction, km_text) so same-km copies link (ME 48xx/49xx/50xx/51xx ↔ CLA-KYN 5TH/6TH; H-41/45/46/47/50 ↔ CSMT-PNVL) while gates (differing km) stay separate; (2b) blank-km automatics by location_text (H-44 'GTBN PF-2', H-48 'CHF PF-2'); (3) leftover → own id. Only writes NULL rows, so existing suburban + 2026-07-14 ghat magnets untouched. **Verified: 0 NULL, within-section 0 violations, cross-section 0 violations, 17 gates NOT merged (GATE-7 trap avoided), 17 automatics linked.** The 34 "ambiguous" groups (17 automatics + 17 gates) were split by the km column (same km=same magnet; differing km=different gate) — user-confirmed. This comprehensive file **supersedes** the ghat-only `2026-07-14_ghat_magnet_ids.sql` for a fresh run. **PROD: run this one file** (prod has all NULL) with the eventual signal-book deploy. |
| Phase 2 (2026-07-31) | CLA-VDLR (BPT / Bombay Port Trust line) imported | Source user master `data/CLA_VDLR/cla_vdlr_cla_master.xlsx` (2 signals tabs + Stations). Sections (ids 88-89): `CLA_VDLR_BPT_UP` 15 sig/18 rows, `CLA_VDLR_BPT_DN` 14 sig/17 rows — **29 signals**. `section='CLA-VDLR'`, lines `BPT UP`/`BPT DN` (Kurla→Wadala). Pre-flight clean first pass. **Shared-signal pattern (like KYN-BSR):** the BPT goods line runs over the harbour + through CLA yard, so most of its signals are same-magnet copies of existing ones — CSMT-PNVL harbour (`CLA S-2/5/12/44/48`, `H-41…H-50`, `RVJ S-24`) and CLA-TMBY yard (`CLA S-31/32/36`, `CLA YD S-2…5/35…38`). Imported as separate CLA-VDLR rows; magnet-link to originals in the deferred pass. **Bound to KYN_GOODS** (34,35) via `sql/2026-07-31_bind_cla_vdlr_to_beats.sql`; renders, 0 orphans. No successors (goods). |
...
| Phase 2 (2026-08-07) | MMR-SNSI branch imported (Manmad→Sainagar Shirdi, off the NE trunk) | Source user master `data/MMR_SNSI/mmr_snsi_master.xlsx` (8 tabs). Sections (ids 94-95): `MMR_SNSI_SNSI_DN` 41 sig/63 rows (7 station + 9 PSR + 6 NS), `MMR_SNSI_SNSI_UP` 41 sig/65 rows (8 + 10 + 6) — **82 signals / 128 rows**. DN `MMR S-88`→`SNSI S-2` (Manmad→Shirdi), UP `SNSI S-16`→`MMR S-33`. Line tag `SNSI DN`/`SNSI UP` (branch off the NE trunk, per the naming rule: trunk=NE, branches get own tag). Stations incl. terminals SAINAGAR SHIRDI (SNSI) / MANMAD (MMR). **Assistant fixes before build (master had sheet-name typos that silently drop aux — .bak kept):** `PSR MMR SNSI N`→`PSR MMR SNSI DN` (missing D), `PSR MMSR SNSI UP`→`PSR MMR SNSI UP` (MMSR typo), ` Stations MMR SNSI UP` leading space trimmed; plus `signal_function='Gate'` (invalid enum) blanked on GATE-77/76/71 (gates are `signal_type='Gate'`, function NULL) and trailing spaces trimmed. Pre-flight then clean, PSR attaches. **Cross-section collision:** only `MMR DIST` (Manmad distant) already in MMR-BSL/UP NE — one-signal-two-books, magnet-linked (both magnet 3599); full backfill re-run, 0 NULL magnet. **Bound to CSMT_ML_MMR** (40,41) via `sql/2026-08-07_bind_mmr_snsi_to_beats.sql`; beat now 41 sections, renders clean. |
| Phase 2 (2026-08-07) | PNVL-JNPT corridor imported (Panvel→JNPT port freight branch) | Source user master `data/PNVL-JNPT/pnvl_jnpt_master.xlsx` (6 tabs: signals/Stations/PSR × DN,UP; no NS, like the other branches). Sections (ids 92-93): `PNVL_JNPT_JNPT_DN` 26 sig/38 rows (5 station + 7 PSR), `PNVL_JNPT_JNPT_UP` 30 sig/42 rows — **56 signals / 80 rows**. DN `PNVL S-9`→`HYD S-35` (Panvel→Holding Yard/JNPT), UP `HYD S-45 DIST`→`PNVL S-26`. Line tag `JNPT DN`/`JNPT UP` (branch convention). Stations: PNVL, DPL(Dapoli), JSLE(Jasai), HYD(Holding Yard), JNPT. **Pre-flight caught (user re-saved):** UP station header anchored to `HYD S-68` which wasn't in the signals (jumps S-67→S-72) — fixed in the re-saved file. **Cross-section collisions:** only the expected Panvel boundary signals (`PNVL S-9/11/12/13/15` DN, `PNVL DIST/S-24/25/26/28/29` UP) already in PNVL-ROHA/PNVL-KJT/PNVL complex — one-signal-two-books, magnet-linked by the full backfill re-run (e.g. `PNVL S-9`→2888, `PNVL S-29`→3053), 0 NULL magnet. **Bound to PNVL_GOODS** (24,25) via `sql/2026-08-07_bind_pnvl_jnpt_to_beats.sql`; renders clean. **Note:** JSLE→BPCL (via DRT) branch off this line — separate future import. |
| Phase 2 (2026-08-06) | KYN-KJT missed signal `ABH S-9` added (UP-line signal, DN move) | Ambernath PF-3 KJT-end starter (id 3603): physically on the **UP line** but governs a **DN** train received on PF-3 being taken to the yard. Modelled `line='UP SE'` (prints on the UP line, grouped with `ABH S-22` PF-3 STR under the AMBERNATH header) + `direction='DN'` (the move it authorises) — its own partition `KYN-KJT/UP SE/DN`, self-magnet 3603. **Option C rendering** (agreed): signal lives once in UP SE (row 5475, after the Ambernath header, before S-22 by km 59.795); a cross-reference `TEXT_NOTE` added at Ambernath in the DN SE line (row 850) — *"DN train received on PF-3 → taken to Yard by ABH S-9 (on UP SE line)"* — so the DN driver, who reads the DN line, is pointed to it (same cross-ref pattern as the PNVL complex). Surgical insert (not a section re-import, to preserve the KYN-KJT lat/long backfill). Also marked `book_description='FOR DN DIRECTION ONLY'` so it prints in the description cell of the UP-line row (an UP reader sees it's a DN-only route). Source `data/KYN_KJT/abh_s9_missed_signal.csv`, applied via `sql/2026-08-06_add_abh_s9_kyn_kjt.sql`; renders clean. |
| Phase 2 (2026-08-06) | MMR-BSL corridor imported (Manmad→Bhusaval — NE trunk continues) | Source user master `data/MMR_BSL/mmr_bsl_master.xlsx` (8 tabs: signals/STATIONS/PSR/NS × DN,UP; standard layout, builder auto-detected). Sections (ids 90-91): `MMR_BSL_DN_NE` 154 sig/213 rows (21 station + 17 PSR + 21 NS), `MMR_BSL_UP_NE` 160 sig/216 rows (21 + 14 + 21) — **314 signals / 429 rows**. DN `MMR S-66`→`BSL S-146`, UP `BSL S-145`→`MMR S-6`. **Line-naming decision (user, 2026-08-06):** NE = the North-East main *trunk* (CSMT→KYN→KSRA→IGP→MMR→**BSL**), so both directions are `DN NE`/`UP NE` (not `BSL DN/UP`); branches off the trunk get their own tag (the coming **MMR-SNSI** branch = `SNSI DN/UP`). **Master fix (user re-saved the file):** DN sheet had a `DN NE`(MMR→NGN)/`BSL DN`(rest) mislabel mix and UP was `BSL UP` — corrected to uniform `DN NE`/`UP NE` across signals + PSR sheets before import. Pre-flight clean (0 dup/enum/orphan). **Cross-section collision check:** only the 2 expected MMR boundary starters — `MMR S-66` (DN) + `MMR S-6` (UP) — already exist in IGP-MMR; one-signal-two-books (like the KYN starter in CSMT-KYN & KYN-KSRA), **magnet-linked** in the bind SQL (`MMR S-66`→2460, `MMR S-6`→2495) and the full backfill re-run gave the other 312 new signals identities (0 NULL magnet remaining, whole DB). **Bound to CSMT_ML_MMR ONLY** (38,39) via `sql/2026-08-06_bind_mmr_bsl_to_beats.sql`; beat now 39 sections/2683 rows, renders clean. Header renames + whole-beat render order = deferred pass. **Still to come:** MMR-SNSI branch (`SNSI DN/UP`), MMR-JL, MMR-BSL onward. |
...
## 23.3 Signal-Book Deployment to Prod (PARKED — status @ 2026-08-06)

Prod audit (2026-08-06, `railway_user` on prod `bbtro`) — **~0% of the signal-book effort is live.**
This whole branch deploys together when it merges; nothing is deployed piecemeal.

| Table | Prod | Local | Gap |
|---|---|---|---|
| `div_signals` | 1503 (baseline) | 2904 | **+1401 to load** |
| `div_signal_aliases` | 1295 | — | ✅ already on prod |
| `div_signal_successors` | 399 | — | ✅ already on prod |
| `div_signal_book_sections` | **absent** | 67 | ❌ create + load |
| `div_signal_beats` | **absent** | 8 | ❌ create + load |
| `div_signal_beat_sections` | **absent** | 151 | ❌ create + load |
| `div_signal_book_rows` | **absent** | 3934 | ❌ create + load |

- Prod `div_signals` **has** the `magnet_id` column (no DDL needed) but it is **unpopulated** — the backfill never ran.
- `display_group` / `lead_in_note` columns don't exist on prod (they live on the missing `div_signal_beat_sections`).
- Lat/long: prod 393/1503 (26%), local 612/2904 (21%). Coordinates exist only on early mainline/NE/SE sections; every corridor imported this year (IGP-MMR, LNL-PUNE, ROHA-RN, PNVL complex, MMR-BSL, …) has **none** yet. Per-section buckets (local): 3 ALL / 7 PARTIAL / 57 NONE.

> **Already on prod ahead of the full deploy:** `ABH S-9` (KYN-KJT/UP SE/DN) was pushed to prod on 2026-08-06 (`sql/2026-08-06_deploy_abh_s9_prod_aws.sql` — div_signals row id **2168** + its `ABHS9` alias only, no book rows) so AWS detects it now. When step 2's full `div_signals` sync runs, it must **upsert/skip** on `uk_signal_section_line` (KYN-KJT / UP SE / ABH S-9), not re-insert. Its book rows (UP SE signal + DN SE cross-ref note) still deploy with the book layer.

### Deploy order (when merging signal-book)
1. **DDL:** create the 4 book tables (`div_signal_book_sections`, `div_signal_beats`, `div_signal_beat_sections`, `div_signal_book_rows`); add `'Intermediate Starter'` to `div_signals.signal_function` (`sql/2026-07-16_add_intermediate_starter_signal_function.sql`) **before importing IGP-MMR data**. (`magnet_id` column already present.)
2. **Data:** bring `div_signals` 1503→2904 and load all 67 sections / 8 beats / 151 bindings / 3934 rows (mysqldump the signal-book tables local→prod; collations match, `utf8mb4_0900_ai_ci`).
3. **Post-load SQL, in order:** all dated `sql/*_bind_*` files → `2026-07-31_magnet_id_backfill_full.sql` → `2026-07-31_add_display_group.sql` → `2026-08-03_add_lead_in_note.sql` → `2026-08-03_pnvl_complex_render_groups.sql` → `2026-08-06_bind_mmr_bsl_to_beats.sql`.
4. **Code:** the signal-book branch's `scripts/render-signal-book.js` + `routes/division/signalBookRoutes.js` (display_group/lead_in_note grouping) must be on the branch prod deploys, then `pm2 restart bbtro`.
5. **Verify:** `/div/signal-book.html` renders a beat; 0 NULL magnet; per-beat section counts match local.
