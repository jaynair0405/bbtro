...
async function renewals(c,req,a,completionId,completionDate,previous={}){const [rules]=await c.query(`SELECT rr.target_id,rr.validity_months,t.legacy_training_id FROM div_training_rule_renewals rr JOIN div_training_renewal_targets t ON t.target_id=rr.target_id WHERE rr.rule_id=?`,[a.rule_id]);for(const r of rules){const due=nextDue(completionDate,r.validity_months);let legacy=previous[r.target_id]||null;if(a.source==='staff'&&r.legacy_training_id){if(legacy){await c.query(`UPDATE div_training_records SET done_date=?,due_date=?,training_center_id=?,status='Completed' WHERE record_id=? AND staff_hrms_id=?`,[completionDate,due,req.trainingCentreId,legacy,a.staff_hrms_id]);}else{const [ins]=await c.query(`INSERT INTO div_training_records(staff_hrms_id,training_id,done_date,due_date,training_center_id,status) VALUES(?,?,?,?,?,'Completed') ON DUPLICATE KEY UPDATE record_id=LAST_INSERT_ID(record_id)`,[a.staff_hrms_id,r.legacy_training_id,completionDate,due,req.trainingCentreId]);legacy=ins.insertId;}}await c.query('INSERT INTO div_training_completion_renewals(completion_id,target_id,due_date,legacy_record_id) VALUES(?,?,?,?)',[completionId,r.target_id,due,legacy]);}}
